...
X_train, X_test, y_train, y_test = train_test_split(df['X_train'], df['y_train'], random_state=RANDOM_STATE,
...
svc = svc_cross_validation(X_train, y_train, scorer)
...
svc_cf_under = svc_cross_validation(X_train_under, y_train_under, scorer)
...
outputCSV(kn.predict(df['X_test']), "multi", "kn")
outputCSV(rf.predict(df['X_test']), "multi", "rf")
outputCSV(svc.predict(df['X_test']), "multi", "svc")
